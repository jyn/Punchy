module ApplicationHelper
	...
	def hours_for(project)
  	...
  	def six_month_client_project_chart(client)
      ...
      GoogleChart::LineChart.new('550x200', "", false) do |lc|
  		  punches.tag_counts_on(:projects).each do |p|
  	      	lc.data p, six_months_of_project_for_client(p, client), random_color
        end
	      lc.show_legend = true
	...
	def six_months_of_project_for_client(project, client)
		a = Array.new
		(0..5).each do |n|
			number = Punch.months_ago(n).tagged_with(client, :as => :client).tagged_with(project, :as => :project).sum(:duration_in_minutes)
			number = number / 60 #convert it to hours
			a << number
		end
		return a.reverse
	...
	def highlight(punch)
		...
		b.gsub!(punch.people_reg) {|s| "<a href='#{people_path(:person => s.gsub("!", ""))}' class='important person'>" + s + '</a>'}
		b
	end
